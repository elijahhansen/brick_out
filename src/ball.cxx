...
{
    auto new_corner = block.top_left();
    auto updated_corner = new_corner.into<int>();
    auto new_middle = updated_corner.right_by(  block.width *.5);
    auto end = new_middle.up_by(1+config.ball_radius);

    return end.into<float>();
...
Ball::top_left() const
{
    ...
    return {Ball::center.x - Ball::radius, Ball::center.y - Ball::radius};
}
...
{
    if ( Ball::center.y + radius > config.scene_dims.height ) {
        return true;
    } else {
...
{
    if (Ball::center.x - radius < 0 || Ball::center.x + radius > config.scene_dims.width)
    {
        return true;
    }
    else
...
{
    if (a.center == b.center && a.live == b.live && a.radius == b.radius && a
    .velocity == b.velocity)
    {
        return true;
    }
